...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, Error> {
        let mut i = 0;
        let mut removed_chunk: Option<Chunk> = None;
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        let header: [u8; 8] = value.drain(..8).collect::<Vec<u8>>().try_into().unwrap();
        ...
        while !value.is_empty() {
            let chunk_length = u32::from_be_bytes(value[..4].try_into().unwrap());
            let chunk = Chunk::try_from(
                &value
                    .drain(..(chunk_length + 12) as usize)
                    .collect::<Vec<u8>>()[..],
            )?;
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.chunks().iter().for_each(|chunk| {
            ...
            write!(f, "{}{}", chunk_type, chunk_data).unwrap();
        });
